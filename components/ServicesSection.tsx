...
import type { HomePage, Service } from "@/sanity/lib/types";

/** Service icons are part of the design; keyed by the service's `icon` field. */
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(services ?? []).map((service) => (
            <div
              key={service._id}
              id={service.slug?.current ?? service.icon}
            ...
            >
              ...
              <ul className="flex flex-col gap-2">
                {(service.homepageFeatures ?? []).map((f) => (
              ...
              {/* Link */}
              <Link
                href={`/services#${service.slug?.current ?? service.icon}`}
